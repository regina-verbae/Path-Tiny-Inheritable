...
isa_ok( $file1, "Path::Tiny::Inheritable" );
ok $file1->isa('Path::Tiny::Inheritable');
is $file1, 'foo.txt';
...
$cat = $dir2->child('foo');
is $cat,     '/tmp/foo';
isa_ok $cat, 'Path::Tiny::Inheritable';
is $cat->dirname, '/tmp/';
...
is $sib,     '/tmp/bar';
isa_ok $sib, 'Path::Tiny::Inheritable';
...

{
    ...
    is( Path::Tiny::Inheritable->cwd,     path( Cwd::getcwd() ) );
...
{
    my $file = Path::Tiny::Inheritable->new( File::Spec->rootdir );
    my $root = Path::Tiny::Inheritable->rootdir;
...
# constructor
{
    is( path(qw/foo bar baz/), Path::Tiny::Inheritable->new(qw/foo bar baz/), "path() vs new" );
...
# freeze/thaw
{
    ...
    is( Path::Tiny::Inheritable->THAW( "fake", $path->FREEZE("fake") ),
...
# assertions
{
    ...
    isa_ok( $path, "Path::Tiny::Inheritable", "assertion return value" );
