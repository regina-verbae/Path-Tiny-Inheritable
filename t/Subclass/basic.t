...
isa_ok( $file1, "MyPathTinySubclass" );
ok $file1->isa('MyPathTinySubclass');
is $file1, 'foo.txt';
...

{
    ...
    is( MyPathTinySubclass->cwd,     path( Cwd::getcwd() ) );
...
{
    my $file = MyPathTinySubclass->new( File::Spec->rootdir );
    my $root = MyPathTinySubclass->rootdir;
...
# constructor
{
    is( path(qw/foo bar baz/), MyPathTinySubclass->new(qw/foo bar baz/), "path() vs new" );
...
# freeze/thaw
{
    ...
    is( MyPathTinySubclass->THAW( "fake", $path->FREEZE("fake") ),
...
# assertions
{
    ...
    isa_ok( $path, "MyPathTinySubclass", "assertion return value" );
